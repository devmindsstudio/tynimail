...
const PagePreview = () => {
  ...
  const { GET_PAGE_BY_ID_PUBLIC } = usePages();
  const { data, isError, isLoading, error } = GET_PAGE_BY_ID_PUBLIC(id ?? "");
  const template = data?.page || null;
  ...
  const sendSessionDuration = async () => {
    ...
    if (sessionDurationSeconds < 1 || !id) return;

    const baseURL = `${import.meta.env.VITE_API_URL}/t/page-session/${id}/${sessionDurationSeconds}`;

    // try {
    //   // navigator.sendBeacon(baseURL);
    // } catch {
    await fetch(baseURL, {
      method: "GET",
      keepalive: true,
  ...
  if (isLoading) return <EmailEditorSkeleton />;
