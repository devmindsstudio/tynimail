...
const PageTemplates = () => {
  ...
  const { GET_ALL_PAGES_TEMPLATE } = usePagesTemplate();

  const { data, isLoading, isError, error } = GET_ALL_PAGES_TEMPLATE();
      ...
      <ul className="grid  sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        {isLoading ? (
          ...
          templatesData.map((template: any, index: number) => {
            const html = JSON.parse(template.content).html;
            return (
              <li
                key={index}
                className="h-70 lg:h-[455px] p-2.5 rounded-2xl bg-card cursor-pointer   overflow-hidden "
              >
                <Link className="" to={`/page-builder/${template.id}`}>
                  <div className=" h-full w-full overflow-hidden">
                    <div
                      className=""
