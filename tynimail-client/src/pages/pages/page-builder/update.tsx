...
const PageBuilderUpdate = () => {
  const navigate = useNavigate();
  const { pageId: templaeId } = useParams<{ pageId: string }>();

  const { GET_PAGE_TEMPLATE } = usePagesTemplate();

  const { data, isError, isLoading, error } = GET_PAGE_TEMPLATE(
    templaeId ?? "",
  );
...
export default PageBuilderUpdate;
