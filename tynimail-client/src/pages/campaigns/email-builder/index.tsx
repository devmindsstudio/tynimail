...
const EmailBuilder = ({
  ...
  const onReady = (ed: Editor) => {
    ...
    // Auto-save listeners for both UPDATE and NEW cases (using USER_UPDATE_TEMPLATE)
    if (getTempateId?.id) {
  ...
  const saveAndContinue = ({ redirect }: { redirect: boolean }) => {
    console.log("user template updatting ]", redirect);
    const ed = editorRef.current;
    if (!ed) return;
    const content = JSON.stringify(editorRef.current?.getProjectData());
    // const hasPreviewCommand = ed.Commands.has("core:preview");
    ...
    console.log("getTempateId", template);
  ...
  // Create USER_TEMPLATE for both UPDATE and NEW cases (only if not already created)
  useEffect(() => {
            ...
            <div className="flex items-center gap-3">
              ...
              {updated ? (
                ...
                <>
                  ...
                  <Button
                    onClick={() => saveAndContinue({ redirect: true })}
                    variant="default"
                    className="h-10 "
                  >
                    Save & Contine
