...
import { EmailBuilderMailyTo } from ".";
import { Button } from "@/components/ui/button";

const EmailBuilderMailyToUpdate = () => {
  const navigate = useNavigate();
  const { templateId } = useParams<{ templateId: string }>();

  const { GET_TEMPLATE_BY_ID } = useTemplates();

  const { data, isError, isLoading, error } = GET_TEMPLATE_BY_ID(
    templateId ?? ""
  );
...
export default EmailBuilderMailyToUpdate;
